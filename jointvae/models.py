...
class VAE(nn.Module):
    def __init__(self, img_size, latent_spec, temperature=.67, use_cuda=False):
        ...
        if self.is_continuous:
            self.fc_alpha = nn.Linear(self.hidden_dim, self.latent_cont_dim)  # 修改的代码
            self.fc_logbeta = nn.Linear(self.hidden_dim, self.latent_cont_dim)   # 修改的代码
        if self.is_discrete:
    ...
    def encode(self, x):
        ...
        if self.is_continuous:
            latent_dist['cont'] = [self.fc_alpha(hidden), self.fc_logbeta(hidden)]  #fc_alpha
    ...
    def reparameterize(self, latent_dist):
        ...
        if self.is_continuous:
            ...
            cont_sample = self.sample_laplace(alpha, logbeta)  # 修改的代码
            latent_sample.append(cont_sample)  # 修改的代码
    ...
    def sample_laplace(self, alpha, logbeta):
        """
        ...
        """
        if self.training:  # 修改后的代码
            beta = torch.div(torch.exp(logbeta*0.5), 2**(0.5))#torch.exp(logbeta*0.5)
            #Lap_distribution = Laplace(0,1) # 得到标准拉普拉斯分布
