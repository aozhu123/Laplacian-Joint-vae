...
class LatentTraverser():
    ...
    def _traverse_continuous_line(self, idx, size):
        ...
        if self.sample_prior:
            samples = np.random.laplace(size=(size, self.cont_dim))
        else:
        ...
        if idx is not None:
            ...
            cont_traversal = stats.laplace.ppf(cdf_traversal)  # 修改的代码
    ...
    def _traverse_continuous_grid(self, idx, axis, size):
        ...
        size : tuple of ints
            Shape of grid to generate. E.g. (6, 4).
        """
        num_samples = size[0] * size[1] # 修改的代码

        if self.sample_prior:
            samples = np.random.laplace(size=(num_samples, self.cont_dim)) # 修改的代码
        else:
        ...
        if idx is not None:
            ...
            cdf_traversal = np.linspace(0.01, 0.99, size[axis])
            cont_traversal = stats.laplace.ppf(cdf_traversal)
        ...
        return torch.Tensor(samples)  # 修改的代码
